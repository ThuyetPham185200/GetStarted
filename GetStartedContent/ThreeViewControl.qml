...
Rectangle {
    ...
    property var iconList: []
     property var colorModel: colorModel
    signal viewChanged()

    function changeItem(index, name) {
        if (index < 0 || index >= iconList.length) return


        // Cập nhật vào colorModel
        colorModel.set(index, { "text_": name })
    }
    Image {
        id: toot
        x: 141
